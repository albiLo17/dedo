...
for task, versions in TASK_INFO.items():
    ...
    if task in ('HangProcCloth', 'HangProcClothReal'):
        versions += versions  # add v2 (two-hole variant)
    if task == 'BGarments':
        bp = os.path.dirname(__file__)
    ...
    if task.startswith('FoodPacking'):
        cls_nm = 'DeformRobotEnv'
    elif task == 'HangProcClothReal':
        cls_nm = 'HangProcClothRealEnv'
    else:
        cls_nm = 'DeformEnv'
