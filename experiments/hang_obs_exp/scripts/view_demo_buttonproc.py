"""
View the scripted hole-aware demo for ButtonProc-v0.

Same template as view_demo_hangbag.py but for the buttoning task: per
episode the script reads both cloth holes' centroids and both button
goals, then plans per-anchor waypoints that drive each hole through
its assigned button. Unlike HangBag, the cloth's far edge is pinned,
so each anchor moves independently (no rigid translation).

Usage (from repo root):
  # Live GUI:
  python experiments/hang_obs_exp/scripts/view_demo_buttonproc.py --viz

  # Save mp4(s) to logs/hang_obs_exp/demo_view_buttonproc/:
  python experiments/hang_obs_exp/scripts/view_demo_buttonproc.py --num_episodes 5

  # Both:
  python experiments/hang_obs_exp/scripts/view_demo_buttonproc.py \
      --viz --num_episodes 3
"""
...
parser.add_argument('--logdir', type=str,
                    default=str(REPO_ROOT / 'logs' / 'hang_obs_exp' /
                                'demo_view_buttonproc'))
parser.add_argument('--seed', type=int, default=42)
parser.add_argument('--env', type=str, default='ButtonProc-v0',
                    help='ButtonProc-v0 re-rerolls textures per reset; '
                         'v1 uses preset textures. Cloth is procedurally '
                         'generated every reset for either version.')
extra = parser.parse_args()

sys.argv = [
    'view_demo_buttonproc',
    f'--env={extra.env}',
    '--cam_resolution', '0',
    '--num_envs=0',
    '--total_env_steps=0',
...
for ep in range(extra.num_episodes):
    ...
    if preset_wp is None:
        print(f'[ep {ep+1}] no loop/goal info available, skipping')
        continue
